...
class BookAlreadyExistsException(HTTPException):
    def __init__(self, title: str, author: str):
        super().__init__(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Book '{title}' by {author} already exists")
...
class EmptyFileException(HTTPException):
    def __init__(self):
        super().__init__(status_code=status.HTTP_400_BAD_REQUEST, detail="File is empty or contains no valid data")
